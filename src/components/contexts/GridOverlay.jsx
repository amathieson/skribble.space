...
const GridOverlay = memo(({ svgRef }) => {
        ...
        const updateBounds = () => {
            ...
            if (parentSvg) {
                ...
                setBounds({
                    x: viewBox.x,
                    y: viewBox.y,
                    width: viewBox.width || parentSvg.clientWidth,
                    height: viewBox.height || parentSvg.clientHeight
                });
            }
        };
    ...
    // Keep getPatternPath inside the component
    const getPatternPath = (gridSizeX, gridSizeY, gridShape) => {
        ...
        switch (gridShape) {
            ...
            case 'hexagon': {
                const stepX = gridSizeX * 0.74;
                const stepY = gridSizeY * 0.51;
                patternWidth = stepX * 2;
                patternHeight = gridSizeY;

                const points = [
                    [gridSizeX * 0.25, 0],
                    [gridSizeX * 0.75, 0],
                    [gridSizeX, gridSizeY * 0.5],
                    [gridSizeX * 0.75, gridSizeY],
                    [gridSizeX * 0.25, gridSizeY],
                    [0, gridSizeY * 0.5]
                ];

                const hexPath = (offsetX = 0, offsetY = 0) => {
                    return points
                        .map((p, i) => `${i === 0 ? 'M' : 'L'} ${p[0] + offsetX} ${p[1] + offsetY}`)
                        .join(' ') + ' Z';
                };

                path = `${hexPath(0, 0)} ${hexPath(stepX, stepY)}`;
                break;
            }
            ...
            default:
                patternWidth = gridSizeX;
                patternHeight = gridSizeY;
                path = `M 0 0 L ${gridSizeX} 0 L ${gridSizeX} ${gridSizeY} L 0 ${gridSizeY} Z`;
                break;
        }
